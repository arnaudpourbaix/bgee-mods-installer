...
export class ModService {
  ...
  getConfig() {
    try {
      const file = fs.readFileSync(this.configFile, {
        encoding: "utf8",
        flag: "r",
      });
  ...
  getMods() {
    try {
      const file = fs.readFileSync(Constants.modsFile, {
        encoding: "utf8",
        flag: "r",
      });
      ...
      console.error(
        chalk.red(`${Constants.modsFile} is not a valid json file!`),
      );
      throw error;
    }
  }
  ...
  async checkExternalMods(): Promise<void> {
    ...
    return this.getExternalMods(config).then((externalMods) =>
  ...
  getExternalMods(config: Config): Promise<ExternalMod[]> {
    ...
    for (const modsFolder of config.modsFolders) {
      if (!fs.existsSync(modsFolder))
        throw new Error(`Path ${modsFolder} doesn't exist`);
      const folders = fs
  ...
  async getGameFolderMods(config: Config): Promise<Mod[]> {
    const folders = fs
      .readdirSync(config.gameFolder, { withFileTypes: true })
      .filter(
        (f) =>
          f.isDirectory() && !Constants.gameFolderIgoreList.includes(f.name),
      )
      .map((f) => f.name);
    const locations = folders
      .map((f) => this.getModLocations(config.gameFolder, f))
      .filter((l) => l !== undefined);
    return Promise.all(locations.map((l) => this.parseMod(config, l))).then(
      (mods) => this.updateModsConfig(config, mods),
    );
  }
  ...
  async parseMod(config: Config, location: ModLocation): Promise<Mod> {
    ...
    return this.getLanguage(config, location)
      .then((r) => {
        language = r;
  ...
  getVersion(location: ModLocation): string {
    const content = fs.readFileSync(location.fullPathTp2File, {
      encoding: "utf8",
      flag: "r",
    });
  ...
  async getLanguage(config: Config, location: ModLocation): Promise<ValueName> {
    ...
    return this.run(cmd, config.gameFolder).then((value) => {
      ...
      let lang = languages.find(
        (l) => l.name.toLowerCase() === config.language.toLowerCase(),
      );
      if (!lang)
        lang = languages.find(
          (l) => l.name === Constants.defaultLanguage.toLowerCase(),
        );
      if (!lang) {
        throw new Error(
          `Language ${config.language} not found for ${location.relativePathFolder}:\n${JSON.stringify(languages)}`,
        );
      }
      return lang;
    });
  ...
  async getComponents(config: Config, location: ModLocation, language: number) {
    ...
    return this.run(cmd, config.gameFolder).then((value) => {
      ...
      const components: ModComponent[] = value
        .matchAll(rx)
        .map((m) => {
  ...
  updateExternalModsConfig(config: Config, mods: ExternalMod[]) {
    for (const mod of mods) {
      ...
      if (!m)
        console.log(
          chalk.magentaBright(`${mod.name} added with version ${mod.version}`),
        );
      else mod.copy = m.copy;
      if (!!m && m.version !== mod.version)
        console.log(
          chalk.blueBright(
            `${mod.name} upgraded from version ${m.version} to ${mod.version}`,
          ),
        );
    }
    config.externalMods = mods.sort(
      (a, b) => (b.copy as unknown as number) - (a.copy as unknown as number),
    );
  ...
  updateModsConfig(config: Config, mods: Mod[]) {
    for (const mod of config.mods) {
      ...
      if (!m)
        console.log(chalk.redBright(`${mod.name} doesn't exist anymore!`));
      else if (m.version !== mod.version) {
        console.log(
          chalk.blueBright(
            `${mod.name} upgraded from version ${mod.version} to ${m.version}, please check all components`,
          ),
        );
    ...
    return this.getGameFolderMods(config).then(() => {
      for (const externalMod of config.externalMods) {
        ...
        if (mod && mod.version === externalMod.version)
          console.log(
            chalk.grey(
              `Skipping ${mod.name}, aready present with the same version`,
            ),
          );
  ...
  copyMod(config: Config, mod: ExternalMod): void {
    ...
    console.log(
      chalk.bold(`Copying ${mod.name}... (${sourceFolder} to ${destFolder})`),
    );
  ...
  async updateModsFromWeiduLog(file: string): Promise<void> {
    ...
    return Promise.resolve().then(() => {
      ...
      for (const group of groups) {
        const modIndex = config.mods.findIndex(
          (m) => m.tp2File.toUpperCase() === group.tp2File,
        );
  ...
  async run(command: string, cwd: string) {
    ...
    return exec(command, { cwd }).then((result) => {
      if (result.stderr) {
        ...
        result.stdout.includes("FATAL ERROR") ||
        result.stdout.includes("Aborting installation of")
      ) {
